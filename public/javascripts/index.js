...
;(function(window, $) {
  ...
  if (todoModel.getSize() > 0) {
    var _template = window.Templates.getTodoTemplate;
    ...
    todos.forEach(function(v) {
       ...
       */
      setTimeout(function() {
        created += _template({task: v.task, limit: v.limit, property: v.priority, check: v.check});
      }, 0);
    });
  ...
  $('#js-todo-regist').on('click', function() {
    ...
    var dto = makeDto({task: $task.val(), limit: $limit.val(), property: $priority.val(), check: false});
  ...
  function createTodoDto($row) {
    ...
    var check    = $row.find('.js-todo-check').prop('checked');
    return makeDto({task: task, limit: limit, priority: priority, check: check});
  }

  function makeDto(obj) {
    /*
